...
/// Plugin that renders splash screen for [`SPLASH_SECS`]. Dependent only on [`assets::SPLASH_PATH`]
/// and [`GameState`].
...
impl<S: States> Plugin for SplashPlugin<S> {
    fn build(&self, app: &mut App) {
        app.init_resource::<SplashTimer>()
            ...
            .add_systems(Update, splash_countdown.run_if(in_state(self.state.clone())))
...
fn setup_splash(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands
        .spawn((
            (BackgroundColor(Srgba::hex("F3F3E8").unwrap().into()), Node {
                width: Val::Vw(100.),
                height: Val::Vh(100.),
